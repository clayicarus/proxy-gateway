...
func TestTrojanFallbackConfiguration(t *testing.T) {
	...
	if fallback.Addr != "127.0.0.1:8080" || fallback.ProbeTimeout != time.Second || fallback.DialTimeout != 3*time.Second || fallback.IdleTimeout != 30*time.Second || fallback.MaxConnections != 128 {
	...
	fallback, err = cfg.Inbounds[0].Trojan.Fallback.WithDefaults()
	if err != nil || fallback.Addr != "[::1]:8080" || fallback.ProbeTimeout != 250*time.Millisecond || fallback.DialTimeout != 2*time.Second || fallback.IdleTimeout != 45*time.Second || fallback.MaxConnections != 8 {
...
func TestTrojanFallbackRejectsInvalidConfiguration(t *testing.T) {
	tests := []struct{ name, replacement, want string }{
		{"missing address", "        idleTimeout: 30s", "fallback.addr must be configured"},
		...
		{"short idle timeout", "        addr: localhost:8080\n        idleTimeout: 100ms", "fallback.idleTimeout"},
		{"long idle timeout", "        addr: localhost:8080\n        idleTimeout: 11m", "fallback.idleTimeout"},
		...
		{"self reference", "        addr: '127.0.0.1:443'", "must not point at the inbound's own listen address"},
	}
