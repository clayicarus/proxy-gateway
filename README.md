...
- After TLS, the complete initial header, including credentials and request, must arrive within `probeTimeout`, also bounded by the original handshake deadline. A matching credential prefix does not extend the window. These timing and resource limits are local configuration choices.
- The gateway adds no decision delay of its own: once a connection is classified it is forwarded to the origin immediately, because every outcome is answered by the origin and the gateway never writes a response itself. A first flight shorter than the 56-character credential plus CRLF cannot be classified until the window expires, so such a connection is forwarded only after `probeTimeout`. Ordinary browser requests are longer than that and are forwarded on arrival.
...
- `addr` comes only from configuration and is never influenced by request content. Startup rejects an `addr` that points at the inbound's own listen address. A hostname is allowed but is re-resolved on every fallback connection, which puts DNS on the probe path; prefer a literal address.
- `maxConnections` bounds anonymous website connections, each of which also holds one backend connection, so match it to the backend's capacity. `idleTimeout` reclaims a website connection that transfers nothing in either direction; a transfer in either direction extends it, so an active visitor is never interrupted mid-download and keep-alive connections are not reset. `dialTimeout` limits dialing. Defaults are shown above. Website traffic has no proxy user, quota or download limiter and is excluded from per-user accounting. Shutdown closes both sides and waits for relay completion.
- A connection that completes TLS but sends no request within `probeTimeout` is closed without taking a website slot or a backend connection, so silent connections cannot lock real visitors out. Browser preconnect sockets are therefore closed once the window expires and have to be reopened.
- When every website slot is busy the Gateway answers with a bodyless `404` carrying no server identity, rather than closing in silence, because a silent close is the signature this fallback removes and slot exhaustion is something a probe can cause on demand. This is the only response the Gateway generates on the website path, and a prober able to compare it with the backend's own 404 under normal load can still tell them apart.
