package server

import (
	"context"
	"crypto/tls"
	"errors"
	"math/rand"
	"net"
	"net/http"
	"sync"
	"sync/atomic"
	"time"

	"github.com/apernet/quic-go"
	"github.com/apernet/quic-go/http3"
	"github.com/apernet/quic-go/quicvarint"

	"github.com/apernet/hysteria/core/v2/internal/congestion"
	"github.com/apernet/hysteria/core/v2/internal/protocol"
	"github.com/apernet/hysteria/core/v2/internal/utils"
)

const (
	closeErrCodeOK                  = 0x100 // HTTP3 ErrCodeNoError
	closeErrCodeTrafficLimitReached = 0x107 // HTTP3 ErrCodeExcessiveLoad
)

type Server interface {
	Serve() error
	Close() error
	Wait(context.Context) error
}

func convertToStdTLSConfig(config *Config) *tls.Config {
	var clientAuth tls.ClientAuthType
	if config.TLSConfig.ClientCAs != nil {
		clientAuth = tls.RequireAndVerifyClientCert
	} else {
		clientAuth = tls.NoClientCert
	}
	return http3.ConfigureTLSConfig(&tls.Config{
		Certificates:   config.TLSConfig.Certificates,
		GetCertificate: config.TLSConfig.GetCertificate,
		ClientCAs:      config.TLSConfig.ClientCAs,
		ClientAuth:     clientAuth,
	})
}

func NewServer(config *Config) (Server, error) {
	if err := config.fill(); err != nil {
		return nil, err
	}
	tlsConfig := convertToStdTLSConfig(config)
	quicConfig := &quic.Config{
		InitialStreamReceiveWindow:     config.QUICConfig.InitialStreamReceiveWindow,
		MaxStreamReceiveWindow:         config.QUICConfig.MaxStreamReceiveWindow,
		InitialConnectionReceiveWindow: config.QUICConfig.InitialConnectionReceiveWindow,
		MaxConnectionReceiveWindow:     config.QUICConfig.MaxConnectionReceiveWindow,
		MaxIdleTimeout:                 config.QUICConfig.MaxIdleTimeout,
		MaxIncomingStreams:             config.QUICConfig.MaxIncomingStreams,
		DisablePathMTUDiscovery:        config.QUICConfig.DisablePathMTUDiscovery,
		EnableDatagrams:                true,
		MaxDatagramFrameSize:           protocol.MaxDatagramFrameSize,
		DisablePathManager:             true,
	}
	listener, err := quic.Listen(config.Conn, tlsConfig, quicConfig)
	if err != nil {
		err = errors.Join(err, config.Conn.Close())
		if config.Cleanup != nil {
			err = errors.Join(err, config.Cleanup.Close())
		}
		return nil, err
	}
	return &serverImpl{
		config:   config,
		listener: listener,
		clients:  make(map[*quic.Conn]struct{}),
	}, nil
}

type serverImpl struct {
	config   *Config
	listener *quic.Listener

	mu        sync.Mutex
	clients   map[*quic.Conn]struct{}
	closing   bool
	closeOnce sync.Once
	closeErr  error
	clientsWG sync.WaitGroup
}

func (s *serverImpl) Serve() error {
	for {
		conn, err := s.listener.Accept(context.Background())
		if err != nil {
			return err
		}
		s.mu.Lock()
		if s.closing {
			s.mu.Unlock()
			_ = conn.CloseWithError(closeErrCodeOK, "")
			continue
		}
		s.clients[conn] = struct{}{}
		s.clientsWG.Add(1)
		s.mu.Unlock()
		go s.handleClient(conn)
	}
}

func (s *serverImpl) Close() error {
	s.closeOnce.Do(func() {
		s.mu.Lock()
		s.closing = true
		clients := make([]*quic.Conn, 0, len(s.clients))
		for conn := range s.clients {
			clients = append(clients, conn)
		}
		s.mu.Unlock()

		s.closeErr = errors.Join(s.listener.Close(), s.config.Conn.Close())
		for _, conn := range clients {
			s.closeErr = errors.Join(s.closeErr, conn.CloseWithError(closeErrCodeOK, ""))
		}
		if s.config.Cleanup != nil {
			s.closeErr = errors.Join(s.closeErr, s.config.Cleanup.Close())
		}
	})
	return s.closeErr
}

func (s *serverImpl) Wait(ctx context.Context) error {
	s.mu.Lock()
	closing := s.closing
	s.mu.Unlock()
	if !closing {
		return errors.New("server Wait called before Close")
	}
	done := make(chan struct{})
	go func() {
		s.clientsWG.Wait()
		close(done)
	}()
	select {
	case <-ctx.Done():
		return ctx.Err()
	case <-done:
		return nil
	}
}

func (s *serverImpl) handleClient(conn *quic.Conn) {
	defer s.clientsWG.Done()
	defer func() {
		s.mu.Lock()
		delete(s.clients, conn)
		s.mu.Unlock()
	}()
	handler := newH3sHandler(s.config, conn)
	h3s := http3.Server{
		Handler:          handler,
		StreamDispatcher: handler.ProxyStreamHijacker,
	}
	err := h3s.ServeQUICConn(conn)
	_ = conn.CloseWithError(closeErrCodeOK, "")
	handler.Wait()
	// If the client is authenticated, we need to log the disconnect event
	session, authID, authenticated := handler.authenticatedSession()
	if authenticated {
		if session != nil {
			if tl := s.config.SessionTrafficLogger; tl != nil {
				tl.LogOnlineStateSession(session, false)
			}
			if el := s.config.SessionEventLogger; el != nil {
				el.DisconnectSession(handler.transport, session, err)
			}
			session.Close(err)
		} else {
			if tl := s.config.TrafficLogger; tl != nil {
				tl.LogOnlineState(authID, false)
			}
			if el := s.config.EventLogger; el != nil {
				el.Disconnect(conn.RemoteAddr(), authID, err)
			}
		}
	}
}

type h3sHandler struct {
	config *Config
	conn   *quic.Conn

	authenticated bool
	authMutex     sync.Mutex
	authID        string
	session       Session
	transport     *transportImpl
	connID        uint32 // a random id for dump streams
	requestID     atomic.Uint64
	workers       sync.WaitGroup

	udpSM *udpSessionManager // Only set after authentication
}

func newH3sHandler(config *Config, conn *quic.Conn) *h3sHandler {
	return &h3sHandler{
		config:    config,
		conn:      conn,
		transport: &transportImpl{conn: conn},
		connID:    rand.Uint32(),
	}
}

func (h *h3sHandler) authenticatedSession() (Session, string, bool) {
	h.authMutex.Lock()
	defer h.authMutex.Unlock()
	return h.session, h.authID, h.authenticated
}

func (h *h3sHandler) Wait() {
	h.workers.Wait()
}

func (h *h3sHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodPost && r.Host == protocol.URLHost && r.URL.Path == protocol.URLPath {
		h.authMutex.Lock()
		defer h.authMutex.Unlock()
		if h.authenticated {
			// Already authenticated
			protocol.AuthResponseToHeader(w.Header(), protocol.AuthResponse{
				UDPEnabled: !h.config.DisableUDP,
				Rx:         h.config.BandwidthConfig.MaxRx,
				RxAuto:     h.config.IgnoreClientBandwidth,
			})
			w.WriteHeader(protocol.StatusAuthOK)
			return
		}
		authReq := protocol.AuthRequestFromHeader(r.Header)
		actualTx := authReq.Rx
		var ok bool
		var id string
		var session Session
		if h.config.SessionAuthenticator != nil {
			session, ok = h.config.SessionAuthenticator.AuthenticateSession(r.Context(), h.transport, authReq.Auth, actualTx)
			if ok && session != nil {
				id = session.ID()
			}
			if session == nil || id == "" {
				ok = false
			}
		} else {
			ok, id = h.config.Authenticator.Authenticate(h.conn.RemoteAddr(), authReq.Auth, actualTx)
		}
		if ok {
			// Set authenticated flag
			h.authenticated = true
			h.authID = id
			h.session = session
			if h.config.IgnoreClientBandwidth {
				// Ignore client bandwidth and use the configured congestion controller.
				congestion.UseConfigured(h.conn, h.config.CongestionConfig.Type, h.config.CongestionConfig.BBRProfile)
				actualTx = 0
			} else {
				// actualTx = min(serverTx, clientRx)
				if h.config.BandwidthConfig.MaxTx > 0 && actualTx > h.config.BandwidthConfig.MaxTx {
					// We have a maxTx limit and the client is asking for more than that,
					// return and use the limit instead
					actualTx = h.config.BandwidthConfig.MaxTx
				}
				if actualTx > 0 {
					congestion.UseBrutal(h.conn, actualTx)
				} else {
					// Client doesn't know its own bandwidth, use the configured congestion controller.
					congestion.UseConfigured(h.conn, h.config.CongestionConfig.Type, h.config.CongestionConfig.BBRProfile)
				}
			}
			// Auth OK, send response
			protocol.AuthResponseToHeader(w.Header(), protocol.AuthResponse{
				UDPEnabled: !h.config.DisableUDP,
				Rx:         h.config.BandwidthConfig.MaxRx,
				RxAuto:     h.config.IgnoreClientBandwidth,
			})
			w.WriteHeader(protocol.StatusAuthOK)
			// Call event logger
			if session != nil {
				if tl := h.config.SessionTrafficLogger; tl != nil {
					tl.LogOnlineStateSession(session, true)
				}
				if el := h.config.SessionEventLogger; el != nil {
					el.ConnectSession(h.transport, session, actualTx)
				}
			} else {
				if tl := h.config.TrafficLogger; tl != nil {
					tl.LogOnlineState(id, true)
				}
				if el := h.config.EventLogger; el != nil {
					el.Connect(h.conn.RemoteAddr(), id, actualTx)
				}
			}
			// Initialize UDP session manager (if UDP is enabled)
			// We use sync.Once to make sure that only one goroutine is started,
			// as ServeHTTP may be called by multiple goroutines simultaneously
			if !h.config.DisableUDP {
				sm := newUDPSessionManager(
					newUDPIOImpl(h, id, session),
					newUDPEventLoggerImpl(h, id, session),
					h.config.UDPIdleTimeout,
					func() uint64 { return h.requestID.Add(1) })
				h.udpSM = sm
				h.workers.Add(1)
				go func() {
					defer h.workers.Done()
					_ = sm.Run()
				}()
			}
		} else {
			// Auth failed, pretend to be a normal HTTP server
			h.masqHandler(w, r)
		}
	} else {
		// Not an auth request, pretend to be a normal HTTP server
		h.masqHandler(w, r)
	}
}

func (h *h3sHandler) ProxyStreamHijacker(ft http3.FrameType, stream *quic.Stream, err error) (bool, error) {
	_, _, authenticated := h.authenticatedSession()
	if err != nil || !authenticated {
		return false, nil
	}

	switch ft {
	case protocol.FrameTypeTCPRequest:
		// StreamDispatcher only peeks the frame type. Consume it so ReadTCPRequest
		// starts at address length, matching pre-upgrade StreamHijacker behavior.
		if _, err := quicvarint.Read(quicvarint.NewReader(stream)); err != nil {
			return false, err
		}
		// Wraps the stream with QStream, which handles Close() properly
		qStream := &utils.QStream{Stream: stream}
		h.workers.Add(1)
		go func() {
			defer h.workers.Done()
			h.handleTCPRequest(qStream)
		}()
		return true, nil
	default:
		return false, nil
	}
}

func (h *h3sHandler) handleTCPRequest(stream *utils.QStream) {
	session, authID, authenticated := h.authenticatedSession()
	if !authenticated {
		_ = stream.Close()
		return
	}
	request := RequestInfo{ID: h.requestID.Add(1), Network: "tcp"}
	trafficLogger := h.config.TrafficLogger
	streamStats := &StreamStats{
		AuthID:      authID,
		ConnID:      h.connID,
		InitialTime: time.Now(),
	}
	streamStats.State.Store(StreamStateInitial)
	streamStats.LastActiveTime.Store(time.Now())
	defer func() {
		streamStats.State.Store(StreamStateClosed)
	}()
	if trafficLogger != nil {
		trafficLogger.TraceStream(stream, streamStats)
		defer trafficLogger.UntraceStream(stream)
	}

	// Read request
	reqAddr, err := protocol.ReadTCPRequest(stream)
	if err != nil {
		_ = stream.Close()
		return
	}
	streamStats.ReqAddr.Store(reqAddr)
	request.Target = reqAddr
	// Call the hook if set
	var putback []byte
	var hooked bool
	if h.config.RequestHook != nil {
		hooked = h.config.RequestHook.Check(false, reqAddr)
		// When the hook is enabled, the server should always accept a connection
		// so that the client will send whatever request the hook wants to see.
		// This is essentially a server-side fast-open.
		if hooked {
			streamStats.State.Store(StreamStateHooking)
			_ = protocol.WriteTCPResponse(stream, true, "RequestHook enabled")
			putback, err = h.config.RequestHook.TCP(stream, &reqAddr)
			if err != nil {
				_ = stream.Close()
				return
			}
			streamStats.setHookedReqAddr(reqAddr)
		}
	}
	// Log the event
	if session != nil {
		if h.config.SessionEventLogger != nil {
			h.config.SessionEventLogger.TCPRequestSession(session, request)
		}
	} else if h.config.EventLogger != nil {
		h.config.EventLogger.TCPRequest(h.conn.RemoteAddr(), authID, reqAddr)
	}
	// Dial target
	streamStats.State.Store(StreamStateConnecting)
	var tConn net.Conn
	if session != nil {
		tConn, err = h.config.SessionOutbound.TCPContext(stream.Context(), session, request)
	} else {
		tConn, err = h.config.Outbound.TCP(reqAddr)
	}
	if err != nil {
		if !hooked {
			_ = protocol.WriteTCPResponse(stream, false, err.Error())
		}
		_ = stream.Close()
		// Log the error
		if session != nil {
			if h.config.SessionEventLogger != nil {
				h.config.SessionEventLogger.TCPErrorSession(session, request, err)
			}
		} else if h.config.EventLogger != nil {
			h.config.EventLogger.TCPError(h.conn.RemoteAddr(), authID, reqAddr, err)
		}
		return
	}
	if !hooked {
		_ = protocol.WriteTCPResponse(stream, true, "Connected")
	}
	streamStats.State.Store(StreamStateEstablished)
	// Put back the data if the hook requested
	if len(putback) > 0 {
		n, _ := tConn.Write(putback)
		streamStats.Tx.Add(uint64(n))
	}
	// Start proxying
	if session != nil && h.config.SessionTrafficLogger != nil {
		err = copyTwoWaySession(stream.Context(), session, request, stream, tConn, h.config.SessionTrafficLogger, streamStats)
	} else if trafficLogger != nil {
		err = copyTwoWayEx(authID, stream, tConn, trafficLogger, streamStats)
	} else {
		// Use the fast path if no traffic logger is set
		err = copyTwoWay(stream, tConn)
	}
	if session != nil {
		if h.config.SessionEventLogger != nil {
			h.config.SessionEventLogger.TCPErrorSession(session, request, err)
		}
	} else if h.config.EventLogger != nil {
		h.config.EventLogger.TCPError(h.conn.RemoteAddr(), authID, reqAddr, err)
	}
	// Cleanup
	_ = tConn.Close()
	_ = stream.Close()
	// Disconnect the client if TrafficLogger requested
	if err == errDisconnect {
		if session != nil {
			session.Close(err)
		}
		_ = h.conn.CloseWithError(closeErrCodeTrafficLimitReached, "")
	}
}

type transportImpl struct {
	conn *quic.Conn
	once sync.Once
	err  error
}

func (t *transportImpl) Context() context.Context { return t.conn.Context() }
func (t *transportImpl) RemoteAddr() net.Addr     { return t.conn.RemoteAddr() }
func (t *transportImpl) Close(cause error) error {
	t.once.Do(func() {
		code := quic.ApplicationErrorCode(closeErrCodeOK)
		if cause != nil {
			code = quic.ApplicationErrorCode(closeErrCodeTrafficLimitReached)
		}
		t.err = t.conn.CloseWithError(code, "")
	})
	return t.err
}

func (h *h3sHandler) masqHandler(w http.ResponseWriter, r *http.Request) {
	if h.config.MasqHandler != nil {
		h.config.MasqHandler.ServeHTTP(w, r)
	} else {
		// Return 404 for everything
		http.NotFound(w, r)
	}
}

// udpIOImpl is the IO implementation for udpSessionManager with TrafficLogger support
type udpIOImpl struct {
	Conn          *quic.Conn
	AuthID        string
	TrafficLogger TrafficLogger
	RequestHook   RequestHook
	Outbound      Outbound
}

func newUDPIOImpl(handler *h3sHandler, authID string, session Session) udpIO {
	legacy := &udpIOImpl{
		Conn:          handler.conn,
		AuthID:        authID,
		TrafficLogger: handler.config.TrafficLogger,
		RequestHook:   handler.config.RequestHook,
		Outbound:      handler.config.Outbound,
	}
	if session == nil {
		return legacy
	}
	return &sessionUDPIOImpl{udpIOImpl: legacy, handler: handler, session: session}
}

func (io *udpIOImpl) ReceiveMessage() (*protocol.UDPMessage, error) {
	for {
		msg, err := io.Conn.ReceiveDatagram(context.Background())
		if err != nil {
			// Connection error, this will stop the session manager
			return nil, err
		}
		udpMsg, err := protocol.ParseUDPMessage(msg)
		if err != nil {
			// Invalid message, this is fine - just wait for the next
			continue
		}
		if io.TrafficLogger != nil {
			ok := io.TrafficLogger.LogTraffic(io.AuthID, uint64(len(udpMsg.Data)), 0)
			if !ok {
				// TrafficLogger requested to disconnect the client
				_ = io.Conn.CloseWithError(closeErrCodeTrafficLimitReached, "")
				return nil, errDisconnect
			}
		}
		return udpMsg, nil
	}
}

func (io *udpIOImpl) SendMessage(buf []byte, msg *protocol.UDPMessage) error {
	if io.TrafficLogger != nil {
		ok := io.TrafficLogger.LogTraffic(io.AuthID, 0, uint64(len(msg.Data)))
		if !ok {
			// TrafficLogger requested to disconnect the client
			_ = io.Conn.CloseWithError(closeErrCodeTrafficLimitReached, "")
			return errDisconnect
		}
	}
	msgN := msg.Serialize(buf)
	if msgN < 0 {
		// Message larger than buffer, silent drop
		return nil
	}
	return io.Conn.SendDatagram(buf[:msgN])
}

func (io *udpIOImpl) Hook(data []byte, reqAddr *string) error {
	if io.RequestHook != nil && io.RequestHook.Check(true, *reqAddr) {
		return io.RequestHook.UDP(data, reqAddr)
	} else {
		return nil
	}
}

func (io *udpIOImpl) UDP(reqAddr string) (UDPConn, error) {
	return io.Outbound.UDP(reqAddr)
}

type sessionUDPIOImpl struct {
	*udpIOImpl
	handler *h3sHandler
	session Session
}

func (io *sessionUDPIOImpl) Admit(request RequestInfo, _ *protocol.UDPMessage, tx, rx uint64) bool {
	if io.session == nil {
		return true
	}
	ok := io.handler.config.SessionTrafficLogger.LogTrafficContext(
		io.handler.transport.Context(), io.session, request, tx, rx)
	if !ok {
		io.session.Close(errDisconnect)
		_ = io.handler.transport.Close(errDisconnect)
	}
	return ok
}

func (io *sessionUDPIOImpl) SendMessageContext(request RequestInfo, buf []byte, msg *protocol.UDPMessage) error {
	if !io.Admit(request, msg, 0, uint64(len(msg.Data))) {
		return errDisconnect
	}
	msgN := msg.Serialize(buf)
	if msgN < 0 {
		return nil
	}
	return io.Conn.SendDatagram(buf[:msgN])
}

func (io *sessionUDPIOImpl) UDPContext(request RequestInfo) (UDPConn, error) {
	return io.handler.config.SessionOutbound.UDPContext(
		io.handler.transport.Context(), io.session, request)
}

type udpEventLoggerImpl struct {
	Conn        *quic.Conn
	AuthID      string
	EventLogger EventLogger
}

func newUDPEventLoggerImpl(handler *h3sHandler, authID string, session Session) udpEventLogger {
	legacy := &udpEventLoggerImpl{Conn: handler.conn, AuthID: authID, EventLogger: handler.config.EventLogger}
	if session == nil {
		return legacy
	}
	return &sessionUDPEventLoggerImpl{udpEventLoggerImpl: legacy, handler: handler, session: session}
}

func (l *udpEventLoggerImpl) New(sessionID uint32, reqAddr string) {
	if l.EventLogger != nil {
		l.EventLogger.UDPRequest(l.Conn.RemoteAddr(), l.AuthID, sessionID, reqAddr)
	}
}

func (l *udpEventLoggerImpl) Close(sessionID uint32, err error) {
	if l.EventLogger != nil {
		l.EventLogger.UDPError(l.Conn.RemoteAddr(), l.AuthID, sessionID, err)
	}
}

type sessionUDPEventLoggerImpl struct {
	*udpEventLoggerImpl
	handler *h3sHandler
	session Session
}

func (l *sessionUDPEventLoggerImpl) NewSession(request RequestInfo) {
	if logger := l.handler.config.SessionEventLogger; logger != nil {
		logger.UDPRequestSession(l.session, request)
	}
}

func (l *sessionUDPEventLoggerImpl) CloseSession(request RequestInfo, err error) {
	if logger := l.handler.config.SessionEventLogger; logger != nil {
		logger.UDPErrorSession(l.session, request, err)
	}
}
