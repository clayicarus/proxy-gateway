...
type Config struct {
	...
	// Gateway session-aware APIs. When SessionAuthenticator is set,
	// SessionOutbound must also be set and the legacy identity handoff is not
	// used. The wire protocol is unchanged.
	SessionAuthenticator SessionAuthenticator
	SessionOutbound      SessionOutbound
	SessionTrafficLogger SessionTrafficLogger
...
func (c *Config) fill() error {
	...
	if c.SessionAuthenticator != nil && c.SessionOutbound == nil {
		return errors.ConfigError{Field: "SessionOutbound", Reason: "must be set with SessionAuthenticator"}
	}
	if c.SessionAuthenticator != nil && c.SessionTrafficLogger == nil {
		return errors.ConfigError{Field: "SessionTrafficLogger", Reason: "must be set with SessionAuthenticator"}
	}
	if c.SessionAuthenticator != nil && c.RequestHook != nil {
		return errors.ConfigError{Field: "RequestHook", Reason: "cannot be used with SessionAuthenticator"}
	}
	if c.Outbound == nil && c.SessionOutbound == nil {
	...
	if c.Authenticator == nil && c.SessionAuthenticator == nil {
...
// Transport is a stable capability for one accepted QUIC connection. It is
// created by the server, cannot be reconstructed from an address, and closes
...
// Session is returned only after successful authentication and remains bound
// to the Transport supplied to SessionAuthenticator.
type Session interface {
	ID() string
	Close(cause error)
}

// RequestInfo identifies one TCP stream or UDP association. ID is generated
// by the server and is unique within the owning transport.
...
type SessionAuthenticator interface {
	AuthenticateSession(ctx context.Context, transport Transport, auth string, tx uint64) (Session, bool)
}

type SessionOutbound interface {
	TCPContext(ctx context.Context, session Session, request RequestInfo) (net.Conn, error)
	UDPContext(ctx context.Context, session Session, request RequestInfo) (UDPConn, error)
}

// SessionTrafficLogger is called at the same payload boundaries as the legacy
// TrafficLogger: each TCP read chunk, each parsed inbound UDP fragment, and
// each outbound UDP send attempt.
type SessionTrafficLogger interface {
	LogTrafficContext(ctx context.Context, session Session, request RequestInfo, tx, rx uint64) bool
	LogOnlineStateSession(session Session, online bool)
}

type SessionEventLogger interface {
	ConnectSession(transport Transport, session Session, tx uint64)
	DisconnectSession(transport Transport, session Session, err error)
	TCPRequestSession(session Session, request RequestInfo)
	TCPErrorSession(session Session, request RequestInfo, err error)
	UDPRequestSession(session Session, request RequestInfo)
	UDPErrorSession(session Session, request RequestInfo, err error)
}
