...
func copyTwoWaySession(
	ctx context.Context,
	session Session,
	request RequestInfo,
	serverRw, remoteRw io.ReadWriteCloser,
	logger SessionTrafficLogger,
	stats *StreamStats,
) error {
	...
	go func() {
		errChan <- copyBufferLog(serverRw, remoteRw, func(n uint64) bool {
			...
			stats.Rx.Add(n)
			return logger.LogTrafficContext(ctx, session, request, 0, n)
		})
	}()
	go func() {
		errChan <- copyBufferLog(remoteRw, serverRw, func(n uint64) bool {
			stats.LastActiveTime.Store(time.Now())
			stats.Tx.Add(n)
			return logger.LogTrafficContext(ctx, session, request, n, 0)
		})
	}()
	...
	_ = remoteRw.Close()
	second := <-errChan
	if errors.Is(first, errDisconnect) || errors.Is(second, errDisconnect) {
		return errDisconnect
	}
