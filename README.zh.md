...
- TLS 成功后，包含凭据和请求的完整初始首部必须在 `probeTimeout` 内到达，且不能超过原有握手 deadline。正确的凭据前缀不会延长窗口。这些时限和资源上限属于本地配置选择。
- Gateway 自身不增加判定延迟：连接一旦判定完成就立即转发到源站，因为所有判定结果都由源站作答，Gateway 从不自己生成响应。首包短于 56 字符凭据加 CRLF 时无法判定，只能等窗口到期后才转发；普通浏览器请求长于这个长度，到达即转发。
- TLS ALPN 只声明 `http/1.1`，该入口生成的 Trojan 订阅包含 `alpn: [http/1.1]`。手动配置客户端时应使用相同设置；不发送 ALPN 的客户端也可连接。不提供 HTTP/2 转换。
- `addr` 只来自配置，永不受请求内容影响；启动时拒绝指回该入站自己的监听地址。允许域名，但每条回退连接都会重新解析一次，相当于把 DNS 放进探测路径，建议用字面地址。
- `maxConnections` 限制匿名网站连接数，每条同时占用一条源站连接，应与源站并发能力匹配；`idleTimeout` 回收双向都没有传输的网站连接，任一方向的传输都会把它顺延，因此活跃访客不会在下载中途被切断，keep-alive 连接也不会被重置；`dialTimeout` 另行限制拨号耗时，默认值如上。网站流量不归属代理用户，不进入用户额度、下载限速和计费统计。停机关闭两端，并等待转发结束。
- TLS 成功但在 `probeTimeout` 内没有发出请求的连接会被直接关闭，不占网站槽位也不拨源站，因此静默连接无法把真实访客挤出去。代价是浏览器的预连接会在窗口到期后被关闭，需要重新建连。
- 网站槽位全忙时 Gateway 返回一个不带 body、不含服务端标识的 `404`，而不是静默关闭：静默关闭正是这个功能要消除的指纹，而槽位耗尽是探测者可以主动造成的。这是网站路径上唯一由 Gateway 生成的响应；能在正常负载下拿到源站自身 404 做对比的探测者仍可区分两者。
