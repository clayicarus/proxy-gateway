...
后续订阅扩展已完成：单入口 `sub.inbound` 也可选择 Trojan，多个公开入口通过 `sub.endpoints[]` 显式列出，各自配置公网地址和 TLS 参数；仍不自动发布未列出的入口。当前契约与示例见 [README](../README.zh.md) 和 [Trojan 设计](TROJAN_INBOUND_DESIGN.md)。以上单 Hy2 限制保留为核心重构阶段的范围说明。
...
- Hy2 入站增加 `masquerade`（`type: proxy`）：未认证的 HTTP/3 请求转发到固定 web 后端，不再一律 404。它只使用 fork 已有的 `MasqHandler` 钩子，不进入认证、会话、路由或账本路径；后端 URL 不受请求内容影响，不发 `X-Forwarded-*`，失败返回空 502，停机时释放连接池。Trojan TCP 网站回退已作为独立功能实现，见 §15。
...
- 请求结构校验通过后才验证凭据并取得用户代理 session。未知凭据和非法初始请求均按 Other Protocols 处理，不产生不同的 Gateway 响应。判定完成后立即转发，Gateway 不增加自己的判定延迟：所有回退结果都由固定源站作答，不存在需要抹平的 Gateway 侧响应时延；首包不足 58 字节时判定本身阻塞到窗口到期，与普通 Web 服务器等待请求剩余部分的行为一致。默认一秒窗口及并发、拨号和连接寿命上限属于资源配置，官方协议不规定其数值；不将它们宣称为官方实现的时延复刻。
- 判定窗口内未发出任何字节的连接不占网站槽位、不拨源站，直接关闭；否则一次握手就能买下槽位和源站连接的整个 relay 寿命，少量静默连接即可挤掉真实访客。此项以牺牲浏览器预连接为代价。
- 网站槽位耗尽时返回 Gateway 生成的无 body、无服务端标识 `404`，这是网站路径上唯一的例外：静默关闭会恢复本功能要消除的指纹，而槽位耗尽可被主动造成。该响应不等同于源站自身的 404，能同时取得两者的探测者仍可区分；后续若需要更强的一致性，应由源站而非 Gateway 生成该响应。
...
- 回退流量不取得用户代理 session，不进入路由、账本、额度、限速、在线状态或请求追踪。独立 `maxConnections` 覆盖等待和 relay，在释放握手槽位前取得网站槽位；`dialTimeout` 限制后端拨号；连接寿命由 `idleTimeout` 以空闲方式约束，任一方向的传输顺延两端 deadline，不得使用绝对期限，否则会在传输中途切断正常访客。后端登记到现有 registry，`Close`/`Wait` 等待双向 relay 结束。日志仅记录固定事件与入口名。
...
- [ ] **TODO-PROBE-02（低优先级）：停用或到期用户的有效 Trojan 凭据会被明文回放到 fallback 源站。**

缺陷记录（2026-09-12）：`auth.Authenticator.AuthenticateTrojan` 对"凭据未知"和"凭据已知但用户停用/到期"都返回 false，Trojan 入站的分类逻辑把两者一视同仁地交给网站 fallback。于是被停用用户的客户端仍会发送当前可派生的有效 SHA-224 凭据，这 56 个十六进制字符连同其后的请求头会作为 HTTP 请求的开头被完整回放给固定源站，落进源站的 access log。

对未知凭据而言这是 fallback 的固有代价，也与上游 trojan-gfw 一致。对已知但非活跃的凭据则是可以避免的：能走到这条路径说明对方已经持有有效凭据，直接关闭连接不会向探测者泄漏任何额外信息，因为探测者本来就构造不出有效凭据。

后续处理至少包括：

- 让 `AuthenticateTrojan` 或其上层能区分"凭据不存在"与"凭据有效但用户不可用"，只对前者回退。注意这个区分不能泄漏到对外可观察的行为上：对未持有凭据的探测者，两者都必须表现为网站。
- 评估停用用户看到网站而不是连接关闭是否会造成误导性的客户端行为（例如客户端把 HTTP 400 当成协议错误反复重连）。
- 在部署文档中说明：只要启用了 fallback，源站日志就可能包含来自旧客户端的凭据材料，源站日志应按凭据材料对待。
- 增加"已知但停用/到期用户走 fallback"的测试；当前 fallback 测试全部使用未知或格式错误的凭据，这条路径没有覆盖。
