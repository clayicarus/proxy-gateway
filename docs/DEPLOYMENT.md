...
| 配置 | 默认值 | 有效范围与含义 |
|---|---|---|
| `probeTimeout` | `1s` | `50ms`–`10s`，TLS 成功后等待凭据的窗口，同时受原始握手 deadline 限制 |
| `dialTimeout` | `3s` | `50ms`–`30s`，后端拨号上限 |
| `timeout` | `30s` | `1s`–`10m`，判定结束后拨号与转发的总时长 |
| `maxConnections` | `32` | `1`–`65535`，等待判定与连接后端的匿名连接总上限 |

格式错误、未知凭据、短首包和读取超时共用判定窗口，已读取的最多 58 字节完整回放且不会重复。因此每条网站 TLS 连接的首次响应默认延迟约一秒；有效 Trojan 凭据也需要在这个窗口内到达，认证成功后的命令解析仍可使用原有握手时限。并发上限耗尽或后端无法连接时关闭连接。已认证但命令非法时仍关闭，不送入网站。
