...
判定遵循 [Trojan 官方协议](https://github.com/trojan-gfw/trojan/blob/3e7bb9aecdc694f9bcae8d646fae395f773d60f8/docs/protocol.md#L49)：只有完整请求结构与凭据同时有效才进入用户代理路径。凭据和请求读取共用有界 tee，保留最多 320 字节，包括 EOF 和超时前的部分读取。未知凭据、非法初始请求和不完整首部都通过 `io.MultiReader` 回放已消费字节并继续转发未读流。判定完成后立即转发，不额外等待窗口结束：所有结果都由固定源站作答，Gateway 不生成响应，因此没有需要抹平的 Gateway 侧时延；首包不足 58 字节时判定本身会阻塞到窗口到期，这与普通 Web 服务器等待请求剩余部分一致。窗口覆盖完整首部，并受原有握手总 deadline 限制；正确凭据前缀不延长窗口。网站回放保留客户端的写半关闭，以允许源站在请求发送完毕后返回响应。
...
TLS 成功后在判定窗口内一个字节都没发出的连接不占网站槽位、不拨源站，直接关闭：它没有可回放的字节也没有要作答的请求，而占用槽位和源站连接的代价是整个 relay 寿命。代价是浏览器的预连接（preconnect）会在窗口到期后被关闭，需要重新建连。

网站槽位耗尽时由 Gateway 直接返回一个不带 body、不含服务端标识的 `404`，而不是静默关闭 —— 静默关闭正是本功能要消除的指纹，而槽位耗尽是探测者可以主动造成的。这是网站路径上唯一由 Gateway 生成的响应；能在正常负载下取得源站自身 404 并做对比的探测者仍可区分两者。

`addr` 在启动时拒绝指回该入站自己的监听地址；允许域名但每条回退连接都会重新解析，因此固定的是配置而不是解析结果。独立 `maxConnections` 覆盖后端 relay；在释放握手槽位前取得网站槽位，防止创建无界等待连接。`dialTimeout` 限制拨号，`idleTimeout` 是空闲上限而不是连接寿命上限：任一方向的传输都会把两端的 deadline 顺延，因此活跃访客不会在传输中途被切断，只有双向静默的连接会被回收；根 context 取消立即中断连接。后端登记到现有 registry，双向 relay 完全退出后才释放 handler。诊断仅记录事件与入口名，不记录首包、疑似凭据、请求路径或原始 TLS 错误。
